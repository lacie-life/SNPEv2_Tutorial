...
def progressive_pruning(pruner, model, speed_up, example_inputs):
    ...
    while current_speed_up < opt.speed_up:
        ...
        if pruner.current_step == pruner.iterative_steps:
            break
        print('current_speed_up %f'%current_speed_up)
    return current_speed_up

def get_pruner(model, example_inputs):
    opt.sparsity_learning = False
    if opt.method == "random":
        ...
        pruner_entry = partial(tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning)
    elif opt.method == "lamp":
        imp = tp.importance.LAMPImportance(p=2)
        pruner_entry = partial(tp.pruner.BNScalePruner, global_pruning=opt.global_pruning)
    elif opt.method == "slim":
    ...
    #opt.is_accum_importance = is_accum_importance
    pruning_ratio_dict = {}
    ...
    pruner = pruner_entry(
        ...
        max_pruning_ratio=opt.max_pruning_ratio,
...
def train(hyp, opt, device, tb_writer=None):
    ...
    progressive_pruning(pruner, model, speed_up=opt.speed_up, example_inputs=example_inputs)
